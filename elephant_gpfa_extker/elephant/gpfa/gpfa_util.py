...
def make_k_big(params, n_timesteps):
    ...
    if params['covType'] in ['rbf','rbf_try']:
        ...
        for i in range(xDim):
            ...
            if params['covType'] == 'rbf_try':
                logdet_K = np.log(np.linalg.det(K))
            else:
                logdet_K = logdet(K)

            logdet_K_big = logdet_K_big + logdet_K

    elif params['covType']=='exp':
        xDim = params['C'].shape[1]
        ...
        for i in range(xDim):
            K = (1 - params['eps'][i]) * np.exp(-(np.abs(Tdif)) / (2 * params['sigma'][i] ** 2)) + params['eps'][
        ...
        for i in range(xDim):
            tau = np.sqrt(6)*params['sigma'][i]
            K = np.zeros((len(Tdif),len(Tdif[0])))
            for row_idx in range(0,len(Tdif)):
                for col_idx in range(0,len(Tdif[row_idx])):
                    if np.abs(Tdif[row_idx][col_idx])<tau:
                        K[row_idx][col_idx] = (1 - params['eps'][i]) *(1/tau)*(1-(np.abs(Tdif[row_idx][col_idx])/tau))+ params['eps'][
                            i] * (row_idx==col_idx)
                    else:
                        K[row_idx][col_idx] = (1 - params['eps'][i]) * 0 + params['eps'][
                            i] * (row_idx==col_idx)
        ...
        for i in range(xDim):
            K = (1 - params['eps'][i]) * np.exp((1+(np.abs(Tdif)**2/2*params['alpha'][i]*(params['ell'][i]**2)))**(-params['alpha'][i])) + params['eps'][
                i] * np.eye(n_timesteps)
            K_big[i::xDim, i::xDim] = K
            K_big_inv[i::xDim, i::xDim] = np.linalg.inv(K)
            logdet_K = logdet(K)
        ...
        r = np.abs(Tdif)
        r = np.array(r)
        r = np.where(r==0,1e-4,r)

        K = np.zeros((len(r), len(r[0])))
        for i in range(xDim):
            for row_idx in range(0,len(r)):
                for col_idx in range(0,len(r[row_idx])):
                    part1 = 2 ** (1 - params['nu'][i]) / gamma(params['nu'][i])
                    part2 = (np.sqrt(2 * params['nu'][i]) * r[row_idx][col_idx] / params['ell'][i]) ** params['nu'][i]
                    part3 = kv(params['nu'][i], np.sqrt(2 * params['nu'][i]) * r[row_idx][col_idx] / params['ell'][i])
                    K[row_idx][col_idx] = (1 - params['eps'][i]) * part1*part2*part3 + params['eps'][
                                              i] * (row_idx == col_idx)

                    # # for matern kernel calculation check
                    # print('nu: '+str(params['nu'][i]))
                    # print('ell: ' + str(params['ell'][i]))
                    # print('part1: '+str(part1))
                    # print('part2: ' + str(part2))
                    # print('part3: ' + str(part3))
                    # print(r[row_idx][col_idx])
        ## matern kernel in the form of matrix 
        # for i in range(xDim):
        #     part1 = 2**(1-params['nu'][i])/gamma(params['nu'][i])
        #     part2 = (np.sqrt(2*params['nu'][i])*r/params['ell'][i])**params['nu'][i]
        #     part3 = kv(params['nu'][i],np.sqrt(2*params['nu'][i])*r/params['ell'][i])
        #     K = (1 - params['eps'][i]) * part1*part2*part3 + params['eps'][
        #         i] * np.eye(n_timesteps)
            # K_big[i::xDim, i::xDim] = K
            # try:
            K_big_inv[i::xDim, i::xDim] = np.linalg.inv(K)
            ...
            #     # K_big_inv[i::xDim, i::xDim] = np.linalg.pinv(K)
            #     print('K',K,)
            #     print('parameters',params['nu'],params['ell'])
            #     print('Dim',i)
        ...
        # K_big_inv = K_big_inv +(np.eye(xDim * n_timesteps)*1.0E-8)

    elif params['covType'] == 'tri_times_rq':
        ...
        for i in range(xDim):
            ...
            for row_idx in range(0, len(Tdif)):
                for col_idx in range(0, len(Tdif[row_idx])):
                    if np.abs(Tdif[row_idx][col_idx]) < tau:
                        K[row_idx][col_idx] = (1 - params['eps'][i]) * (1 / tau) * (1 - (np.abs(Tdif[row_idx][col_idx]) / tau))\
                        * np.exp((1+(np.abs(Tdif[row_idx][col_idx])**2/2*params['alpha'][i]*(params['ell'][i]**2)))**(-params['alpha'][i]))\
                                              + params['eps'][i] * (row_idx == col_idx)
                    else:
                        K[row_idx][col_idx] = (1 - params['eps'][i]) * 0 + params['eps'][
                            i] * (row_idx == col_idx)
    ...
    elif params['covType']=='exp_times_rq':
        xDim = params['C'].shape[1]
        ...
        for i in range(xDim):
            K = (1 - params['eps'][i]) * (np.exp(-(np.abs(Tdif)) / (2 * params['sigma'][i] ** 2))*
                                          np.exp((1+(np.abs(Tdif)**2/2*params['alpha'][i]*(params['ell'][i]**2)))**(-params['alpha'][i]))) + params['eps'][
                i] * np.eye(n_timesteps)
            K_big[i::xDim, i::xDim] = K
            # K = K + (np.eye(n_timesteps)*(1e-8))
            K_big_inv[i::xDim, i::xDim] = np.linalg.inv(K)
            logdet_K = logdet(K)

            logdet_K_big = logdet_K_big + logdet_K

    elif params['covType'] == 'exp_times_tri':
        ...
        for i in range(xDim):
            tau = np.sqrt(6) * params['sigma_tri'][i]
            K = np.zeros((len(Tdif), len(Tdif[0])))
            for row_idx in range(0, len(Tdif)):
                for col_idx in range(0, len(Tdif[row_idx])):
                    if np.abs(Tdif[row_idx][col_idx]) < tau:
                        K[row_idx][col_idx] = (1 - params['eps'][i]) *(np.exp(-(np.abs(Tdif[row_idx][col_idx])) / (2 * params['sigma_exp'][i] ** 2))*
                                     (1 / tau) * (1 - (np.abs(Tdif[row_idx][col_idx]) / tau)) ) + params['eps'][i] * (row_idx == col_idx)
                    else:
                        K[row_idx][col_idx] = (1 - params['eps'][i]) * 0 + params['eps'][
                            i] * (row_idx == col_idx)
        ...
        for i in range(xDim):
            ...
            for j in range(params['Q']):
                K = K + params['w'][i][j] * np.exp(-2 * np.pi**2 * params['vs'][i][j]**2 * diffSq) * np.cos(2 * np.pi * Tdif.T * params['mu'][i][j]) 
            ...
            # slogdet returns the natural log of the ABSOLUTE value of the determinant.
